...
function embed(parent, svg, target, use, doc) {
...
function loadreadystatechange(xhr, use, doc) {
	...
	xhr.onreadystatechange = function () {
		...
		if (xhr.readyState === 4) {
			...
			if (!cachedDocument) {
				cachedDocument = xhr._cachedDocument = doc.implementation.createHTMLDocument('');
			...
			xhr._embeds.splice(0).map(function (item) {
				...
				embed(item.parent, item.svg, target, use, doc);
			});
		}
	};
...
function svg4everybody(rawopts) {
	...
	// Assign document element
	var doc = 'document' in opts ? opts.document : document;
	...
	var uses = doc.getElementsByTagName('use');
				...
				if (LEGACY_SUPPORT && nosvg) {
					...
					if (!opts.validate || opts.validate(src, svg, use)) {
						...
						if (url.length) {
							...
							loadreadystatechange(xhr, use, doc);
						} else {
							// embed the local id into the svg
							embed(parent, svg, doc.getElementById(id), use, doc);
						}
					} else {
