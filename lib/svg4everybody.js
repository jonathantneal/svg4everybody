...
function embed(parent, svg, target, doc) {
...
function loadreadystatechange(xhr, doc) {
	...
	xhr.onreadystatechange = function () {
		...
		if (xhr.readyState === 4) {
			...
			xhr._embeds.splice(0).map(function (item) {
				...
				embed(item.parent, item.svg, target, doc);
			});
		}
	};
				...
				if (LEGACY_SUPPORT && nosvg) {
					...
					if (!opts.validate || opts.validate(src, svg, use)) {
						...
						if (url.length) {
							...
							loadreadystatechange(xhr, doc);
						} else {
							// embed the local id into the svg
							embed(parent, svg, doc.getElementById(id), doc);
						}
					} else {
